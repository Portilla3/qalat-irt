...
def _detectar_centros(wide_path):
    ...
    try:
        ...
        # Columna A es separador vacío, centros están en columna B (índice 1)
...
def run_paquetes_centros(wide_path, keys_sel=None, progress_cb=None, raw_bytes=None, raw_input_path=None):
    if keys_sel is None:
    ...
    # Crear archivo raw temporal desde los bytes si están disponibles
    _raw_tmp = None
    if raw_bytes:
        fd, _raw_tmp = tempfile.mkstemp(suffix='.xlsx', prefix='qalat_irt_raw_')
        os.close(fd)
        with open(_raw_tmp, 'wb') as f:
            f.write(raw_bytes)
    elif raw_input_path and os.path.exists(raw_input_path):
        _raw_tmp = raw_input_path
    ...
    try:
        with zipfile.ZipFile(zip_buf, 'w', zipfile.ZIP_DEFLATED) as zf:
            for i, centro in enumerate(centros):
                ...
                try:
                    if _raw_tmp and os.path.exists(_raw_tmp):
                        from pipeline.wide_irt import procesar_wide as _pw
                        res_c = _pw(_raw_tmp, filtro_centro=centro)
        ...
        # Limpiar raw temporal si lo creamos aquí
        if raw_bytes and _raw_tmp:
            try: os.unlink(_raw_tmp)
            except: pass
